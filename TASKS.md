...
| 11 | Frontend shell &amp; end-to-end integration | [#11](https://github.com/B-a-y-a-n-d-a/Stats-SA/issues/11) | [specs/011](specs/011-frontend-shell-integration/spec.md) | 4-10 | Done | Claude |

**Status: all 11 tasks are merged to `master`.** This is the full MVP — every Mandatory Functional Capability has a real, live-verified implementation behind it, not a mock. Full backend suite is 106/106 passing; `demo/run-demo.md` walks the complete 6-step demo script, and it was run for real against the actual `docker-compose up` stack (not `.venv`/`npm run dev`) end to end before this task merged. Next step for the team is demo rehearsal, not more building — see `demo/run-demo.md`.

**Task 11 wired the 5 views into one app** — a real role switcher (`POST /api/auth/login`, gating `/review`/`/curator`/`/audit` to match the actual server-side RBAC, not a UI-only illusion) and a shared theme/styles module so the app reads as one product instead of five independently-styled prototypes (`MediaIntake.tsx` had no styling at all before this). The real payoff of this task was walking the literal `docker-compose up` stack end to end for the first time all hackathon, which surfaced integration bugs no amount of code review or `.venv`-based testing would have caught: the frontend's production image had no `/api` proxy at all (fixed with nginx), the backend image was pulling a 1GB+ unused-CUDA `torch` build, and the backend's Docker build context couldn't see the demo seed document. All fixed and re-verified live — see specs/011 for the full list and the step-by-step demo verification notes.
...
**Recurring lesson, now that the whole build is done:** twice (Tasks 5+9, then independently again in Task 7's parallel build) two people/agents writing similar test files in parallel, without seeing each other's code, duplicated logic that should have been shared — first the `app.dependency_overrides.clear()` bug, now a duplicated serializer function. Neither was harmful, both were caught and fixed, but it's the one pattern worth remembering for any future work on this codebase: if you're building something with a natural shared piece (a serializer, a helper), search for it first. Task 11's own dedup (`frontend/src/styles/`) was the fix for the same pattern having already happened once, across every view.
...
- Task 11: a production Docker build is not the same environment as `.venv`/`npm run dev` — three separate, real bugs only surfaced by actually running `docker-compose up`: the frontend's static server had no `/api` proxy at all (100% of API calls would 404), the backend pulled a 1GB+ unused-CUDA `torch` wheel, and the backend's Docker build context couldn't see the demo seed document outside `backend/` (PR #23)
- Task 11: a live-Postgres test (`test_retrieval.py`) assumed it was the only writer to the database — after a live demo run wrote real `audit_log` rows, its own cleanup fixture broke on a `ForeignKeyViolation` deleting a still-referenced source (PR #23)
