...
| 9 | RBAC &amp; auth | [#9](https://github.com/B-a-y-a-n-d-a/Stats-SA/issues/9) | [specs/009](specs/009-rbac-auth/spec.md) | 1 | In Progress (branch pushed, issue not yet self-assigned) | Devin |
...
**Status:** Tasks 1 and 2 are merged to `master`, both fully live-verified against a real Postgres container. Two real bugs were caught by that live testing (not visible from code review alone): passlib's bcrypt backend breaks against modern `bcrypt` (PR #13), and a models.py enum-encoding mismatch that only surfaces once you actually insert a `Source` row (fixed in PR #15). Someone (git identity "Devin") has already pushed `feature/009-rbac-auth` without assigning themselves issue #9 first — if that's you or a teammate, please assign yourself on the issue so this table and the board stay accurate; if it's an AI agent working in parallel, same rule applies. Tasks 3, 4 (after 3), 5 (after 3), 8, 10 are open to claim now.
